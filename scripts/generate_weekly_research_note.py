...
def weekly_sentiment(rows: list[dict]) -> dict:
    if len(rows) < 2:
        return {"label_ko": "확인 필요", "label_en": "Needs data", "detail_ko": "개인 수급 데이터가 충분하지 않습니다.", "detail_en": "Retail-flow data is not sufficient."}

    latest = rows[-1]
    prev = rows[-6] if len(rows) >= 6 else rows[-2]
    ret = pct_change(latest["close"], prev["close"])
    ...
    if ret < -2 and recent_flow < 0:
        return {
            "label_ko": "공포 쪽에 가까움",
            "label_en": "Near fear",
            "detail_ko": f"최근 KOSPI가 {fmt_pct(ret)} 움직인 가운데 최근 5거래일 개인 순매수는 {fmt_num(recent_flow)}조원입니다.",
    ...
    if ret > 2 and recent_flow > 0:
        return {
            "label_ko": "탐욕 쪽에 가까움",
            "label_en": "Near greed",
            "detail_ko": f"최근 KOSPI가 {fmt_pct(ret)} 상승했고 최근 5거래일 개인 순매수는 {fmt_num(recent_flow)}조원입니다.",
            "detail_en": f"KOSPI rose {fmt_pct(ret)} recently, and five-session retail net-buying was {fmt_num(recent_flow)}T KRW.",
        }
    return {
        "label_ko": "중립",
        "label_en": "Neutral",
        "detail_ko": f"최근 KOSPI 변화율은 {fmt_pct(ret)}, 최근 5거래일 개인 순매수는 {fmt_num(recent_flow)}조원입니다.",
...
def ai_capex_summary(data: dict) -> dict:
    ...
    tone = "여유" if (avg_ocf or 9) <= 0.7 and (avg_profit or 9) <= 0.9 else "관리 가능" if (avg_ocf or 9) <= 1.0 and (avg_profit or 9) <= 1.2 else "부담 확대"
    tone_en = {"여유": "comfortable", "관리 가능": "manageable", "부담 확대": "burden rising"}[tone]
    return {"avg_ocf": avg_ocf, "avg_profit": avg_profit, "tone_ko": tone, "tone_en": tone_en}
...
<body>
  ...
  <main>{body}</main>
  <footer><p>DISCLAIMER · Educational reference only · Not investment advice · Data may be delayed or incomplete</p><nav class="footer-links"><a href="{depth}en/notes/">Research Notes</a><a href="{depth}privacy/">Privacy</a><a href="{depth}disclaimer/">Disclaimer</a><a href="{depth}contact/">Contact</a></nav></footer>
</body>
</html>
"""
...
def load_existing_posts(index_path: Path) -> list[tuple[str, str, str]]:
    if not index_path.exists():
        return []
    html = index_path.read_text(encoding="utf-8")
    posts = []
    for slug, title in re.findall(r'href="([0-9]{4}-[0-9]{2}-[0-9]{2})/"><span>([^<]*)</span><strong>([^<]*)</strong>', html):
        posts.append((title, slug, title))
    return []
...
def list_weekly_posts(base_dir: Path) -> list[tuple[str, str, str]]:
    posts = []
    if base_dir.exists():
        for child in sorted(base_dir.iterdir(), reverse=True):
            ...
            title = f"{child.name} 주간 시장 리서치 노트"
            if index.exists():
...
def main() -> None:
    ...
    if forced_date:
        now = datetime.fromisoformat(f"{forced_date}T09:00:00+09:00")
    else:
    ...
    <section class="article-body">
      <p>이번 주 WIP Labs는 KOSPI 밸류에이션, 개인 수급 심리, 미국 유동성, AI CAPEX 부담을 함께 점검합니다. 이 글은 매주 일요일 자동으로 생성되는 시장 리서치 노트이며, 투자 권유가 아니라 스스로 판단하기 위한 체크리스트입니다.</p>
      <h2>1. KOSPI 밸류에이션</h2>
      <p>최근 KOSPI 기준일은 {kospi['date']}입니다. 현행 PER는 {fmt_num(kospi['per'])}배, 2010년 이후 평균 PER는 {fmt_num(kospi['historicalAveragePer'])}배입니다. 현행 PER는 평균보다 {fmt_num(abs(per_gap))}배 {'높습니다' if per_gap >= 0 else '낮습니다'}. Forward PER 참고치는 {fmt_num(FORWARD_PER)}배로, 예상 이익 기준의 할인 여부와 이익 전망 신뢰도를 함께 봐야 합니다.</p>
      <h2>2. 개인 수급 심리</h2>
      <p>최근 심리 상태는 <strong>{sentiment['label_ko']}</strong>입니다. {sentiment['detail_ko']} 수급 신호는 단독 매매 신호가 아니라 공포와 탐욕의 온도를 보는 보조 지표입니다.</p>
      <h2>3. 변동성</h2>
      ...
      <h2>4. 미국 유동성</h2>
      <p>미국 유동성 종합 판단은 <strong>{liquidity_summary.get('label')}</strong>입니다. 최근 약 {liquidity.get('lookbackDays')}일 기준 우호 지표는 {liquidity_summary.get('positives')}/{liquidity_summary.get('total')}개이며, 실질 유동성 보조값 변화는 {fmt_num(liquidity_summary['marketLiquidity'].get('change'))}B USD입니다.</p>
      <h2>5. AI CAPEX 부담</h2>
      <p>하이퍼스케일러의 평균 CAPEX/OCF는 {fmt_pct((ai['avg_ocf'] or 0) * 100)}, 평균 CAPEX/순이익은 {fmt_pct((ai['avg_profit'] or 0) * 100)}입니다. 현재 분류는 <strong>{ai['tone_ko']}</strong>입니다. AI 투자 사이클은 성장의 신호이지만, 현금흐름 안에서 감당되는지 계속 확인해야 합니다.</p>
      <h2>이번 주 체크포인트</h2>
    ...
    <section class="article-body">
      ...
      <p>The U.S. liquidity read is <strong>{'supportive' if liquidity_summary.get('tone') == 'positive' else liquidity_summary.get('tone')}</strong>. {liquidity_summary.get('positives')}/{liquidity_summary.get('total')} indicators are supportive over roughly {liquidity.get('lookbackDays')} days, and the real-liquidity proxy changed by {fmt_num(liquidity_summary['marketLiquidity'].get('change'))}B USD.</p>
    ...
    (ko_weekly_dir / "index.html").write_text(weekly_index(list_weekly_posts(ko_weekly_dir), en=False), encoding="utf-8")
    (en_weekly_dir / "index.html").write_text(weekly_index(list_weekly_posts(en_weekly_dir), en=True), encoding="utf-8")
