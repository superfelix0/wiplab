...
const fmtDateTime = new Intl.DateTimeFormat(IS_EN ? "en-US" : "ko-KR", {
  year: "2-digit",
  month: "2-digit",
  day: "2-digit",
  hour: "2-digit",
...
function parseCsv(text) {
  ...
  if (dateIndex < 0 || closeIndex < 0 || flowIndex < 0) {
    throw new Error(t("CSV에는 date, close, indiv_krw 열이 필요합니다.", "CSV needs date, close, and indiv_krw columns."));
  }
...
function formatUpdateMeta(meta, latestDate) {
  ...
  if (meta?.generatedAt) {
    return t(
      `자료 기준 ${dataDate} · 수집 ${fmtDateTime.format(new Date(meta.generatedAt))}`,
      `Data date ${dataDate} · collected ${fmtDateTime.format(new Date(meta.generatedAt))}`
    );
  }
  return t(`자료 기준 ${dataDate}`, `Data date ${dataDate}`);
}
...
function latestSentiment(point) {
  if (point.type === "fear") {
    return {
      tone: "fear",
      label: t("공포 신호", "Fear signal"),
      short: t("공포", "Fear"),
      note: t(
        `개인 순매수가 평소 예상보다 ${Math.abs(point.residual).toFixed(2)}조원 부족했습니다.`,
        `Retail net-buying was ${Math.abs(point.residual).toFixed(2)}T KRW below its usual estimate.`
      ),
    };
  }
  ...
  if (point.type === "greed") {
    return {
      tone: "greed",
      label: t("탐욕 신호", "Greed signal"),
      short: t("탐욕", "Greed"),
      note: t(
        `개인 순매수가 평소 예상보다 ${Math.abs(point.residual).toFixed(2)}조원 많았습니다.`,
        `Retail net-buying was ${Math.abs(point.residual).toFixed(2)}T KRW above its usual estimate.`
      ),
    };
  }
  ...
  if (Math.abs(point.z) < 0.25) {
    return {
      ...
      short: t("중립", "Neutral"),
      note: t(
        "개인 순매수가 평소 움직임에서 크게 벗어나지 않았습니다.",
        "Retail flow is not far from its usual pattern."
      ),
    };
  }
  ...
  if (point.z < 0) {
    return {
      tone: "fear-soft",
      label: t("공포 쪽에 가까움", "Near fear"),
      short: t("공포 근접", "Near fear"),
      note: t(
        `정식 공포 신호는 아니지만 개인 순매수가 예상보다 ${Math.abs(point.residual).toFixed(2)}조원 부족했습니다.`,
        `Not a formal fear signal, but retail net-buying was ${Math.abs(point.residual).toFixed(2)}T KRW below estimate.`
      ),
    };
  }
  ...
  return {
    tone: "greed-soft",
    label: t("탐욕 쪽에 가까움", "Near greed"),
    short: t("탐욕 근접", "Near greed"),
    note: t(
      `정식 탐욕 신호는 아니지만 개인 순매수가 예상보다 ${Math.abs(point.residual).toFixed(2)}조원 많았습니다.`,
      `Not a formal greed signal, but retail net-buying was ${Math.abs(point.residual).toFixed(2)}T KRW above estimate.`
    ),
  };
}
...
function filterRowsByRange(rows) {
  ...
  if (selectedRange === "1y") cutoff.setFullYear(cutoff.getFullYear() - 1);
...
function renderMiniLine(rows, color = "#6fbf73", label = t("변동성 최근 3개월 추이", "3-month volatility trend")) {
...
function volatilityCard({ label, value, date, history, source, color = "#6fbf73" }) {
  ...
  const volatilityDescription = t(
    `${label}는 연율화된 변동성 지수입니다. 지수값을 1년 거래일 수의 제곱근(약 15.87)으로 나누면 대략적인 하루 예상 변동률을 추정할 수 있습니다. 실제 하루 변동폭을 보장하지는 않습니다.`,
    `${label} is an annualized volatility index. Dividing the index level by the square root of roughly 252 trading days estimates a rough one-day expected move. It is not a guarantee of actual daily movement.`
  );
  return `
    <article class="featured" data-tone="volatility">
      <span>${label}</span>
      <strong>${fmt.format(value)}</strong>
      ${renderMiniLine(Array.isArray(history) ? history.slice(-66) : [], color, `${label} ${t("최근 3개월 추이", "3-month trend")}`)}
      <small>${date} · ${t("최근 3개월 추이", "3-month trend")}. ${volatilityDescription} ${t("현재 기준", "Current estimate")}: ±${dailyExpectedMove.toFixed(2)}%/${t("일", "day")}.${source ? ` ${t("출처", "Source")}: ${source}` : ""}</small>
    </article>
  `;
}
...
function renderSummary(analysis) {
  ...
  const modeLabel = dataMode === "live" ? t("실데이터", "Actual data") : t("합성 미리보기", "Demo preview");
  const rangeLabel = sentimentEls.range?.selectedOptions?.[0]?.textContent || "1Y";
  const updateLabel = formatUpdateMeta(dataMeta, latest.date);
  const latestView = latestSentiment(latest);
  const residualDirection = latest.residual < 0 ? t("부족", "below") : t("초과", "above");
  const kospiChange = previous ? ((latest.close / previous.close) - 1) * 100 : null;
  const volatility = dataMeta?.kospi200Volatility;

  const cards = [
    card({
      label: t("최근 심리", "Latest sentiment"),
    ...
    card({
      ...
      sub: `${selectedFreq === "W" ? t("주간", "Weekly") : t("일간", "Daily")} ${t("수익률", "return")} ${latest.ret.toFixed(2)}%${kospiChange === null ? "" : ` · ${t("직전 대비", "vs previous")} ${kospiChange >= 0 ? "+" : ""}${kospiChange.toFixed(2)}%`}`,
    }),
    card({
      label: t("개인 순매수", "Retail net buying"),
      value: `${latest.indivT.toFixed(2)}${t("조원", "T KRW")}`,
      sub: `${rangeLabel} · ${t("평소 예상", "usual estimate")} ${latest.expected.toFixed(2)}${t("조원 대비", "T KRW,")} ${Math.abs(latest.residual).toFixed(2)}${t("조원", "T KRW")} ${residualDirection}`,
      tone: latest.residual < 0 ? "fear-soft" : "greed-soft",
      wide: true,
    }),
    card({
      label: t("데이터 상태", "Data status"),
      value: modeLabel,
      sub: updateLabel,
    }),
  ];
...
function renderDetail(point) {
  ...
  const miss = point.residual < 0 ? t("부족", "below") : t("초과", "above");

  sentimentEls.detail.innerHTML = IS_EN ? `
    <strong>${point.date} · ${latestView.label}</strong>
    <p>KOSPI closed at ${fmt.format(point.close)}, and the selected period return was ${point.ret.toFixed(2)}%.</p>
    <p>Given that return, the usual pattern would estimate retail net-buying at about ${point.expected.toFixed(2)}T KRW. Actual retail net-buying was ${point.indivT.toFixed(2)}T KRW, or ${Math.abs(point.residual).toFixed(2)}T KRW ${miss} the estimate.</p>
    <p>${latestView.note} This is not a standalone buy or sell signal. It is a supporting frame for spotting when market psychology has moved away from its usual pattern.</p>
  ` : `
    <strong>${point.date} · ${latestView.label}</strong>
...
async function fetchSentimentData() {
    ...
    try {
      ...
      if (rows.length >= 120) {
        ...
        return {
          ...
          meta,
          mode: "daily-csv",
          message: t(`데이터 불러오기 성공. ${updateLabel}.`, `Data loaded successfully. ${updateLabel}.`),
        };
      }
    } catch {
  ...
  if (!response.ok || !data?.ok) {
    throw new Error(t("실데이터를 준비하는 중입니다.", "Actual data is being prepared."));
  }
  ...
  return {
    ...
    meta,
    mode: "live-api",
    message: t(`실데이터 API를 불러왔습니다. ${formatUpdateMeta(meta, data.rows.at(-1)?.date)}.`, `Loaded live API data. ${formatUpdateMeta(meta, data.rows.at(-1)?.date)}.`),
  };
}
...
async function fetchVixData() {
  ...
  const data = await response.json().catch(() => null);
  if (!response.ok || !data?.ok) {
    throw new Error(data?.message || t("VIX 데이터를 불러오지 못했습니다.", "Could not load VIX data."));
  }
  return data;
}
...
async function loadData() {
  ...
  setSentimentStatus(t("데이터를 확인하는 중입니다.", "Checking data."));
