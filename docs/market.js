...
function renderPercentileScale(perData, dailyState = null) {
  const dashboard = document.querySelector(".market-dashboard");
  if (!dashboard) return;
  const kospi = perData.markets.kospi200;
  const values = (kospi.history || []).map((row) => Number(row.per)).filter(Number.isFinite).sort((a, b) => a - b);
  if (!values.length) return;
  const percentileFor = (value) => values.filter((item) => item <= value).length / values.length * 100;
...
async function loadMarketDashboard() {
  ...
  try {
    ...
    renderPercentileScale(perData, dailyState);
